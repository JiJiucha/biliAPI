...
def get_cards(uids,cookie:cookieClass.Cookie=cookieClass.null_cookie):
    return mrequests.get(get_user_cards,params={'uids':','.join([str(i) for i in uids])},cookie=cookie)
def get_card(uid,cookie:cookieClass.Cookie=cookieClass.null_cookie):
    return mrequests.get(get_user_card,params={'mid':uid},cookie=cookie)
def get_info(uid,cookie:cookieClass.Cookie=cookieClass.null_cookie):
    return mrequests.get(get_user_info,params={'mid':uid},cookie=cookie,withwbi=True)
    
def get_my_info(cookie:cookieClass.Cookie=cookieClass.null_cookie):
