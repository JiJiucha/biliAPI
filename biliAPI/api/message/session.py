...
def get_session_list(cookie::cookieClass.Cookie,session_type=0,size=20,group_fold=0,unfollow_fold=0,sort_rule=0,begin_ts='',end_ts=''):
...
def get_session_push(cookie::cookieClass.Cookie,talker_uid):
    return mrequests.get(get_session_limit_stat,cookie=cookie,params={'talker_uid':talker_uid})

def set_session_read(cookie::cookieClass.Cookie,session_type,talker_id,ack_seqno=''):
...
#从会话列表移除，不会删除历史记录
def remove_session(cookie::cookieClass.Cookie,session_type,talker_id):
...
def set_session_top(cookie::cookieClass.Cookie,session_type,talker_id,is_top):
...
def set_session_dnd(cookie::cookieClass.Cookie,session_type,vid,dnd):
    uid=''
    group_id=''
...
def set_session_push(cookie::cookieClass.Cookie,talker_id,push):
...
def set_session_intercept(cookie::cookieClass.Cookie,talker_id,intercept):
